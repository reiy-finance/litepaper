...
Sui hosts a mature DeFi stack, spanning automated market makers, a native central
limit order book in DeepBook, aggregators, lending, and derivatives, on an object
centric, parallel execution base with low latency and cheap fees \cite{suidefistack}.
...
The ecosystem does not lack places to trade; it lacks a layer that turns this
breadth into one efficient market rather than a set of isolated venues.

\subsection{Fragmentation and Slippage}

Liquidity is fragmented: each venue prices a pair against its own partial view of
depth, so capital that could clear a trade jointly sits divided, and a trader
seeking best execution must compare venues and split orders by hand. Fragmentation
also fixes a floor on slippage that no venue can remove while its liquidity stays
siloed; the deeper the split, the worse the realized price and the more value leaks
to arbitrage and reordering rather than to the user.

\subsection{Why Existing Intent Markets Fall Short}

Intent based trading addresses this: a user states a desired outcome, such as a
minimum to receive, and delegates execution to competing agents. CoW Protocol
demonstrates the design on Ethereum, matching opposing orders directly and
aggregating external liquidity to beat single venue execution
\cite{documentationcowdaoe}. Its weakness is competition itself: solver
participation costs restrict entry and can drive the market toward oligopoly,
eroding the execution quality intents promise
\cite{chitra2024analysisintentbasedmarkets}. A viable intent system must therefore
make permissionless solver participation a first class goal. None exists natively on
Sui, and Ethereum designs do not transfer to its object centric model.
...
This section fixes the primitives and the research question; readers fluent in Sui
and intent trading may skip ahead.

\subsection{The Sui Execution Model}

Reiy relies on two features of Sui's object model \cite{suidefistack}. A
\emph{shared object} holds escrowed user assets that any party may act on under
contract enforced rules. And a Programmable Transaction Block (PTB) composes several
Move calls into one atomic transaction, where Move's linear \emph{hot potato} type
forces a value created mid block to be consumed before the block ends. Together they
let Reiy escrow funds on chain and settle a batch in a single all-or-nothing
transaction; Section~5.6 develops the hot potato in detail.
...
Reiy depends on the Sui venue landscape in one structural way: it reads a canonical
on chain price. DeepBook, the native order book, exposes a per pool mid price that a
contract can query directly \cite{querypoolsui}, which Reiy uses as the admission
reference of Section~5. How a solver then sources liquidity is outside the protocol.

\subsection{Intents, Solvers, and Batch Auctions}

A user signs an \emph{intent}, a desired outcome such as a minimum to receive, and
delegates execution to competing \emph{solvers} \cite{documentationcowdaoe}; batching
intents admits a coincidence of wants, where opposing orders clear without price
impact and only the residual is sourced externally.

In prevailing designs the intent and all coordination around it live off chain, and
only settlement touches the chain; that off chain locus is what affords stable latency
and low cost. This frames Reiy's central question: can an intent be escrowed and
enforced on chain, making custody and user protection trustless, while keeping the
latency and cost of an off chain market? Reiy answers yes, by escrowing the intent on
chain and confining the off chain coordinator to selection and signing. Its second
requirement, permissionless solver participation against the oligopoly tendency of
intent markets \cite{chitra2024analysisintentbasedmarkets}, is made safe by on chain
...
\emph{settlement transaction}, governed by a \emph{uniform clearing price} that keeps
each batch fair. We define each in turn and then describe epoch progression. The
construct names and parameters follow the Move implementation.
...
\]
with a positive fill not exceeding the intent's remaining amount. Within a directed
market the payouts must additionally form a uniform clearing price (Section~5.4),
so solvers compete by raising that price, not by discriminating between users. Valid
solutions are ranked by score and the highest wins, ties broken deterministically.
Because admission is permissionless and ranking depends only on score, any outside
solver can win by offering strictly better execution, which is the structural
countermeasure to the entry barriers that drive intent markets toward oligopoly
\cite{chitra2024analysisintentbasedmarkets}.

\subsection{Fairness: the Uniform Clearing Price}

\begin{definition}[Clearing price]
A clearing price is a single exchange rate at which every intent on the same
directed market in a batch settles, so that no user is filled at a worse price than
another on that market.
\end{definition}

\noindent The per intent floors of Section~5.2 protect each user in isolation but
say nothing about how users on the same market fare relative to one another, which
would let a solver cherry pick: fill one order at an excellent price and another at
only its floor. Reiy removes that discretion with a uniform clearing price, the
fairness rule of CoW Protocol \cite{documentationcowdaoe}. Let intent $i$ sell $x_i$
and receive $p_i$. A solution settles a directed market at a single price $\pi$ when
\[
  p_i = x_i\,\pi \quad\text{for every intent } i \text{ on the market,}
\]
equivalently when $p_i / x_i$ is equal across them. Two conditions bind $\pi$. It
must clear every floor, $p_i \ge \mathit{protected\ min}_i$, so
$\pi \ge \max_i (\mathit{protected\ min}_i / x_i)$: the tightest floor per unit sold
sets the lowest admissible price. And competition drives it upward, since a solver
wins by delivering more total surplus,
\[
  \sum_i \bigl(p_i - \mathit{protected\ min}_i\bigr)
    = \pi \sum_i x_i - \sum_i \mathit{protected\ min}_i,
\]
which grows with $\pi$. The winning solution is the highest uniform price the batch's
sourced liquidity can support.

\begin{figure}[htbp]
  \centering
  \resizebox{0.9\columnwidth}{!}{%
  \begin{tikzpicture}[font=\footnotesize, >={Stealth[length=2mm]}]
    \draw[->, line width=0.5pt] (0,0) -- (6,0) node[right, font=\scriptsize]{amount sold $x$};
    \draw[->, line width=0.5pt] (0,0) -- (0,4) node[above, font=\scriptsize]{received $p$};
    \draw[cOn, line width=1pt] (0,0) -- (5.4,3.7)
      node[above right, font=\scriptsize, cOn]{clearing price $\pi$};
    \draw[cOff, dashed, line width=0.8pt] (0,0) -- (5.4,2.75)
      node[right, font=\scriptsize, cOff]{$\pi'$ too low};
    \foreach \x/\f/\n in {1.4/0.72/A, 2.9/1.62/B, 4.3/2.6/C} {
      \pgfmathsetmacro\p{0.685*\x}
      \fill[cOn] (\x,\p) circle (2pt) node[above left, font=\scriptsize]{\n};
      \fill[cSup] (\x,\f) circle (1.6pt);
      \draw[cSup, dashed] (\x,\f) -- (\x,\p);
    }
    \node[cSup, font=\scriptsize, anchor=north] at (4.3,2.6) {floor};
    \node[font=\scriptsize\itshape, cSup] at (2.0,3.5) {one price for all, each above its floor};
  \end{tikzpicture}}
  \caption{Uniform clearing price. Every intent on a market lies on one price ray
    $\pi$, and the gap from each user's floor up to the ray is its surplus. A lower
    price $\pi'$ is infeasible because it would breach the tightest floor.}
  \label{fig:clearing}
\end{figure}

The Contract enforces uniformity on chain. For the intents of one directed market it
checks that $p_i / x_i$ is equal within a small tolerance, a cross multiplication
$p_i\,x_j = p_j\,x_i$ that avoids division, alongside the per intent floor check. Fair
pricing is therefore not a matter of solver honesty but a verified on chain invariant:
every user on a market receives the same price, and that price still clears each
user's SBBO floor.

This is the uniform clearing price of a batch auction, produced here by permissionless
competition rather than assumed after the fact. A fair combinatorial auction shows that
competition among solvers can construct such a fairness benchmark endogenously, from the
solvers' own bids, without the protocol needing to know the fair price in advance
\cite{canidio2025faircombinatorialauctiona}.
...
Two properties of Sui make this possible. A PTB threads the typed result of one
Move call into the next, so a value produced by verification flows through the calls
that release escrow and deliver payouts without leaving the transaction. The
\emph{hot potato} pattern makes that threading mandatory: a Move value whose type
has none of \texttt{copy}, \texttt{drop}, \texttt{store}, or \texttt{key} cannot be
copied, discarded, or stored, only unpacked by a function in its defining module, so
a value created mid PTB obligates a later call to consume it or the transaction is
ill formed. Settlement thus becomes a type system guarantee rather than a matter of
trust.

Concretely, \texttt{take\_authorized\_intent} returns the escrowed sell coin and a
\texttt{SettlementReceipt} with no abilities, a true hot potato consumable only by
\texttt{settle\_intent}. The moment a solver takes an intent's escrow it holds a
receipt it cannot drop, store, or move outside the transaction, so it must call
\texttt{settle\_intent} with the exact certified payout in the same PTB. No escrow
leaves the contract without a matching settlement, for every intent in the batch at
once.
...
Reiy monetizes a slice of the surplus a competitive solver network delivers, not
the trader's principal. Because the fee is bounded so the net never drops below the
protected minimum (Section~7.1), revenue comes only from value the network created.
The business is positive sum: Reiy earns only when a trader is filled better.

Revenue has two hedging streams. The \emph{base fee} scales with settled volume and
holds a floor even when competition drives surplus toward zero; the \emph{surplus
fee} scales with delivered price improvement and peaks when execution is best. As
competition sharpens, per trade surplus thins but volume grows, so revenue does not
collapse in the equilibrium that best serves users.

Unit economics are favorable: revenue per trade is capped at 0.25\% of notional while
the protocol's marginal settlement cost is near zero, since the solver pays gas and
the coordinator is a fixed cost. The real cost base is not settlement but the
incentive that keeps solvers competitive.

That incentive is paid in REIY, not from the fee, so early growth runs ahead of
revenue: emissions bootstrap a competitive solver set before fee revenue is material,
and solvers earn REIY on top of the execution margin they capture. As volume grows,
treasury funded buyback replenishes the reward pool and emissions taper. The
crossover, where buyback covers the rewards needed to retain solvers, marks the shift
from a subsidized to a self funding network (Figure~\ref{fig:crossover}).
...
Value compounds (Figure~\ref{fig:flywheel}): better execution attracts flow, flow
yields fees, fees fund rewards and buyback, and deeper rewards draw more solvers. REIY
captures the loop, with fee funded buyback underpinning its value, solver bonds locking
its supply, and holders governing the parameters. A treasury of real traded assets
backs the token, not the reverse.
...
The sensitivities are bounded. Surplus varies with market conditions, which is why
the base fee stabilizes revenue; fees accrue in a basket of assets, requiring a
treasury conversion policy; and the crossover depends on volume growth, so until it
is reached the reward pool leans on emissions and their dilution. Each is a governable
parameter, set conservatively under a multisig at launch and handed to REIY governance
...
Reward funding and value accrual follow the loop of Section~7.2. The treasury is
initially held by an \texttt{AdminCap} under a multisig, transitioning to on chain
governance as the network matures.
...
\begin{property}[Fair pricing]
All intents on the same market settle at one clearing price that clears every floor,
so no user is cherry picked.
\end{property}
\noindent The Contract verifies that the payout to sell ratio is equal across a
market by cross multiplication, so uniform pricing is an on chain invariant rather
than a matter of solver discretion (Section~5.4).
