...
In other words, Sui no longer lacks places to trade. The supply of exchange
models is broad and, for most users, sufficient. What the ecosystem still lacks
is a layer that turns this breadth into a single efficient market rather than a
set of isolated ones.
...
We present Reiy, the first intent based meta DEX aggregator on Sui. A user
submits an intent whose assets are escrowed on chain, and a permissionless
network of solvers competes in epoch gated auctions to fill it at the best price
by matching a coincidence of wants and by sourcing external liquidity. Reiy
adapts the design of CoW Protocol \cite{documentationcowdaoe} to Sui, where fast
finality and low fees allow the auction and settlement loop to run at a cadence
the underlying chain sustains.

Reiy is deliberately not another DEX. It neither competes with nor replaces the
venues already on Sui, but sits above them as an execution layer that turns their
fragmented liquidity into depth a single trade can reach.

The remainder of this paper develops the design. Section~2 fixes the
preliminaries and the research question, Section~3 states the system model, and
Section~4 gives the architecture. Section~5 defines the core constructs: intents,
the SBBO floor, solvers, certificates, and the Programmable Transaction Block
settlement that binds them. Section~6 describes the off chain coordinator,
Section~7 the economic model and the REIY token, and Section~8 the resulting
security properties.
...
In prevailing designs the intent and the entire coordination around it, including
the auction and the choice of solver, live off chain; only the final settlement
touches the chain. This off chain locus is precisely what affords stable latency
and low cost, because the expensive coordination never pays for on chain
execution. The design tension Reiy addresses is therefore the following. Can an
intent be escrowed and enforced on chain, so that custody and user protection
become trustless, while retaining the latency and cost of an off chain market?
Reiy answers in the affirmative by escrowing the intent on chain and confining
the off chain coordinator to selection and signing, leaving custody and
correctness to the contract.

A second, independent tension concerns competition. A formal analysis of intent
markets shows that the cost of participating as a solver restricts entry and can
drive the market toward a small oligopoly, eroding the execution quality intents
promise \cite{chitra2024analysisintentbasedmarkets}. Reiy treats open and
permissionless solver participation as a first class requirement, and Section~5
shows how on chain verification makes it safe to admit any solver. Whether such a
market converges to competitive rather than oligopolistic behavior in practice
...
on chain rule enforcer; the Fisherman is trusted only for liveness, with its
authority bounded by what the Contract accepts; traders, solvers, and external
liquidity are untrusted and verified on chain.

The Fisherman's bound is the central safety property. It cannot custody or move
escrowed funds, since custody resides solely in the Contract; it cannot settle a
user below the protected minimum, admit an intent beneath the SBBO floor, or cause
a solver to win without a certificate the Contract independently verifies. A
malicious or offline Fisherman can at most stall progress, a liveness loss
recovered through permissionless epoch advance and fallback, and can never cause
loss of funds. Safety thus rests on the Contract alone, and the Fisherman bears
only on liveness.

Adversarial cases reduce to this separation. Solver collusion is countered by
permissionless entry, since any outside party can undercut a colluding group;
...
A trade moves through one epoch as a logical sequence of phases. The Contract
keeps the design small: it escrows intents, verifies Certificates, and settles,
and it does not run a phase machine on chain. The phase sequencing itself is
orchestrated off chain by the Fisherman. In \emph{Intent Collection} users submit
intents whose sell assets are escrowed in the Contract, each admitted only if it
clears its SBBO floor. In \emph{Solving} bonded solvers build settlement plans
over the batch, matching Coincidence of Wants and routing residual volume through
Sui liquidity. In \emph{Selection} the Fisherman ranks the valid solutions and
signs a Certificate for the winner. In \emph{Settlement} the winning solver
executes the certified plan in a single PTB, and the Contract verifies the
Certificate and every payout floor before any asset moves. In \emph{Close} the
epoch is finalized and fees are split. If no valid solution arrives, or the
winner fails to settle in time, the epoch advances through a permissionless
fallback and the affected intents return to the next batch, so the protocol never
stalls.
...
Each design goal of Section~3.3 is enforced at a specific point in this
lifecycle. The SBBO floor protects users at admission (Section~5.2);
permissionless competition and ranking occur during solving and selection
(Section~5.3); and certificate verified, atomic settlement enforces the protected
minimums and the all or nothing guarantee at settlement (Sections~5.4 and~5.5).
The economic model of Section~7 then aligns solver and protocol incentives without
touching protected funds.
...
the entire batch in one PTB, shown in Figure~\ref{fig:settle}, so the
verification, the release of escrow, and the delivery of payouts either all commit
or all revert.
...
\texttt{protected\_min} is at least the intent's own minimum. It hands back the
escrowed sell coin and a \texttt{SettlementReceipt} that carries no drop ability.
That receipt is a hot potato: the PTB cannot end until it is consumed by
\texttt{settle\_intent}, which forces the solver to actually settle every intent
it takes.

Third, \texttt{settle\_intent} requires that the delivered buy coin equals the
certified gross payout exactly, confirms the solver is active and bonded, deducts
fees in a way that keeps the net at or above the protected minimum, and transfers
the net to the trader. If any check in the PTB fails, the entire transaction
reverts and no escrowed asset moves. Settlement is therefore atomic and the
trader's floor holds even after fees.
...
The economic model has two parts: a fee that is value aligned and never zero, and
the REIY token that secures and rewards the solver network. We present the
mainnet design; the current testnet contracts use a simpler in kind fee split and
a SUI bond, which mainnet supersedes.
